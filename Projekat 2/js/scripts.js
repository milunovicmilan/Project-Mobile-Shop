   ...
   const viewOptionsButtons14Plus = document.querySelectorAll(".view-options-btn-14plus");
    ...
    $(document).ready(function () {
        ...
        $(document).on('click', function (event) {
            if (!$(event.target).closest('.modal-content').length && $(event.target).hasClass('modal')) {
                $('#addToCartModal').modal('hide');
            }
        });
    
        // Ensure modal is hidden properly when clicking outside
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    function adjustFontSize(scale) {
        ...
        root.style.fontSize = `${currentSize * scale}px`;
    }
});
